...
本リポジトリは Docker Compose 前提で開発する。以下は **絶対遵守**。
...
- Docker内で実行する場合は例: `docker compose exec <frontend_service> <command>`

### 2.3 バックエンド（例）

- 依存/起動/検証はリポジトリ定義に準拠
- Docker内で実行する場合は例: `docker compose exec <backend_service> <command>`
